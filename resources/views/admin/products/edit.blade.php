...
@section('title', 'Edit Produk - Admin NeroPC')

@section('content')
    <section id="form-section" class="form-section">
      <div class="section-header">
        <h2>Edit Produk: {{ $product->name }}</h2>
        <a href="{{ route('admin.products.index') }}" class="btn-secondary" style="font-size: 0.8rem; padding: 6px 12px;">
            Kembali ke Katalog
        </a>
      </div>
      ...
      <form id="form-pc" action="{{ route('admin.products.update', $product->id) }}" method="POST" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="form-grid form-grid-3">
          <div class="form-group">
            ...
            <input type="text" id="sku" name="sku" value="{{ old('sku', $product->sku) }}" placeholder="cth: CPU-INTEL-I5" autocomplete="off" required>
            <span class="error-msg" id="error-sku"></span>
          </div>
          <div class="form-group">
            <label for="name">Nama Produk <span class="required">*</span></label>
            <input type="text" id="name" name="name" value="{{ old('name', $product->name) }}" placeholder="cth: Intel Core i5-13400F" autocomplete="off" required>
          ...
          <div class="form-group">
            ...
            <select id="category" name="category" required>
              ...
              <option value="VGA" {{ old('category', $product->category) == 'VGA' ? 'selected' : '' }}>VGA</option>
              <option value="Storage" {{ old('category', $product->category) == 'Storage' ? 'selected' : '' }}>Storage</option>
              <option value="PSU" {{ old('category', $product->category) == 'PSU' ? 'selected' : '' }}>PSU</option>
              <option value="Casing" {{ old('category', $product->category) == 'Casing' ? 'selected' : '' }}>Casing</option>
              <option value="Prebuilt PC" {{ old('category', $product->category) == 'Prebuilt PC' ? 'selected' : '' }}>Prebuilt PC (Paket Rakitan)</option>
            </select>
        ...
        <div class="form-grid form-grid-3">
          <div class="form-group">
            ...
            <input type="text" id="brand" name="brand" value="{{ old('brand', $product->brand) }}" placeholder="cth: Intel / ASUS / MSI" autocomplete="off" required>
            <span class="error-msg" id="error-brand"></span>
          </div>
          <div class="form-group">
            <label for="price">Harga (Rp) <span class="required">*</span></label>
            <input type="number" id="price" name="price" value="{{ old('price', $product->price) }}" placeholder="cth: 2850000" min="0" required>
            <span class="error-msg" id="error-price"></span>
          </div>
          <div class="form-group">
            <label for="stock">Stok <span class="required">*</span></label>
            <input type="number" id="stock" name="stock" value="{{ old('stock', $product->stock) }}" placeholder="cth: 15" min="0" required>
        ...
        <div class="form-group">
          ...
          <textarea id="description" name="description" rows="3" placeholder="Tulis deskripsi singkat produk di sini...">{{ old('description', $product->description) }}</textarea>
        </div>

        <div class="form-options" style="margin: 15px 0;">
            <label class="remember-me">
                <input type="checkbox" name="is_recommended" id="is_recommended" {{ old('is_recommended', $product->is_recommended) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Attach listener to existing rows
